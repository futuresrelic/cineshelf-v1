...
// ENHANCED: Function to find backup files with strict format checking
function findBackupFiles($user) {
    $files = [];
    
    // Ensure data directory exists
    if (!file_exists('data')) {
        error_log("CineShelf Restore: Data directory does not exist");
        return $files;
    }
    ...
    error_log("CineShelf Restore: Looking for exact file: {$exactFile}");
    
    if (file_exists($exactFile)) {
        $files[] = $exactFile;
        error_log("CineShelf Restore: Found exact match: {$exactFile}");
    }
    
    // Look for any other files with correct prefix that contain the username
    $allFiles = glob("data/cineshelf_backup_*.json");
    error_log("CineShelf Restore: Found " . count($allFiles) . " total backup files with correct format");
    ...
    foreach ($allFiles as $file) {
        ...
        if (strpos($userPart, $user) !== false && !in_array($file, $files)) {
            $files[] = $file;
            error_log("CineShelf Restore: Added matching file: {$file}");
        }
    }
    ...
    error_log("CineShelf Restore: Final file list: " . implode(', ', $files));
    return $files;
}
...
if ($forceFile) {
    ...
    error_log("CineShelf Restore: Force file requested: {$forceFile} -> {$filename}");
    
    if (!file_exists($filename)) {
        error_log("CineShelf Restore: Forced file not found: {$filename}");
    ...
    if (empty($availableFiles)) {
        error_log("CineShelf Restore: No files found for user: {$user}");
    ...
    error_log("CineShelf Restore: Selected file: {$filename}");
}

// Read and return the backup data
$data = file_get_contents($filename);
if ($data === false) {
    error_log("CineShelf Restore: Failed to read file: {$filename}");
...
if ($jsonData === null) {
    error_log("CineShelf Restore: Invalid JSON in file: {$filename}");
    http_response_code(500);
...
error_log("CineShelf Restore: SUCCESS - Restored from {$filename} for user {$user}");
