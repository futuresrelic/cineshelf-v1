        ...
        if (!this.userId) {
            try {
                const response = await fetch(`${this.apiUrl}/init`, {
                    ...
                    body: JSON.stringify({ display_name: 'Movie Collector' })
                });
...
window.App = (function() {
    ...
    let users = ['default'];
    let customEditions = [];
    let lastSelectedMovie = null; // Track last movie selected from search
    let currentSort = {
        collection: 'title',
        wishlist: 'title'
    };
    ...
    function setupEventListeners() {
        ...
        if (movieTitle) {
            movieTitle.addEventListener('input', function() {
                ...
                // Clear last selected movie when user manually edits title
    ...
    function handleFormSubmit(e) {
        ...
        const formData = {
            ...
            format: document.getElementById('format').value || 'Unknown',
        ...
        // Use the last selected movie if available, otherwise try to find by title
        let matchingMovie = null;
        
        if (lastSelectedMovie && lastSelectedMovie.title.toLowerCase() === formData.title.toLowerCase()) {
            // Use the movie that was just selected from search
            matchingMovie = lastSelectedMovie;
        } else {
            // Fall back to title search (for manual entries)
            matchingMovie = movies.find(m => 
        ...
        // Clear last selected movie after adding
        lastSelectedMovie = null;
...
function updateFilterCounter(type, showing, total) {
    const counter = document.getElementById(`${type}FilterCounter`);
    if (counter) {
        if (showing === total) {
            counter.textContent = `${total} movies`;
            counter.style.opacity = '0.7';
        } else {
            counter.textContent = `Showing ${showing} of ${total}`;
            counter.style.opacity = '1';
        }
    }
}

function setLetterFilter(type, letter) {
    currentFilter[type].letter = letter;
    
    // Update active button
    const buttons = document.querySelectorAll(`#${type} .letter-filter-btn`);
    buttons.forEach(btn => {
    ...
    const rating = movie?.imdbRating ? movie.imdbRating.toFixed(1) : 'N/A';
        ...
        card.innerHTML = `
            ...
            <div class="movie-card-content">
                ...
                <div class="movie-info">
                    ...
                    <span class="movie-card-format"> • ${copy.format}</span>
                    ${rating !== 'N/A' ? `<br><span class="movie-card-rating">⭐ ${rating}</span>` : ''}
                    ${runtime ? `<br><span class="movie-card-runtime">⏱️ ${runtime}</span>` : ''}
                    ${director ? `<br><span class="movie-card-director">🎬 ${director}</span>` : ''}
                    ${genre ? `<br><span class="movie-card-genre">${genre}</span>` : ''}
                </div>
            </div>
        `;
    }
    ...
    function startEditMovie(copyId) {
        ...
        currentResolveItem = null;
        lastSelectedMovie = null; // Clear last selected movie when editing
    ...
    // ========================================
    // SIMPLIFIED BACKUP/RESTORE - SINGLE PHP FILES ONLY
    // ========================================
    ...
    async function backupToServer() {
        ...
        try {
            ...
            console.log('CineShelf: Backing up', data.copies.length, 'items for user:', currentUser);
            
            // Use ONLY root backup.php
            const response = await fetch('backup.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-User-ID': currentUser,
                    'X-Backup-Version': '3.0'
                },
            ...
            if (result.success) {
                serverStatus.innerHTML = `✅ Backup successful!<br>${result.items_count} items saved to ${result.filename}`;
                serverStatus.className = 'status success';
                serverStatus.style.display = 'block';
                setTimeout(() => serverStatus.style.display = 'none', 4000);
                
                console.log('CineShelf: Backup success:', result);
            } else {
            ...
            serverStatus.innerHTML = `❌ Backup failed: ${error.message}`;
...
async function restoreFromServer(forceFile = null) {
        ...
        try {
            ...
            const serverCopies = data.copies.length;
            ...
            const confirmMessage = `Found backup from ${backupDate}\n\nServer has: ${serverCopies} items, ${serverMovies} movies, ${serverCustomEditions} custom editions\nYour device has: ${copies.length} items, ${movies.length} movies\n\n⚠️ This will REPLACE all data on this device.\n\nContinue?`;
            ...
            if (confirm(confirmMessage)) {
                ...
                serverStatus.innerHTML = `✅ Restore successful!<br>${copies.length} items restored from ${fileUsed}`;
                serverStatus.className = 'status success';
                serverStatus.style.display = 'block';
                setTimeout(() => serverStatus.style.display = 'none', 6000);
                
                console.log('CineShelf: Restore success');
            }
            
        } catch (error) {
            console.error('Restore error:', error);
            serverStatus.innerHTML = `❌ Restore failed: ${error.message}`;
            serverStatus.className = 'status error';
            serverStatus.style.display = 'block';
            setTimeout(() => serverStatus.style.display = 'none', 8000);
        }
    ...
    function showAvailableBackups(backups) {
        ...
        if (!backups || backups.length === 0) {
            serverStatus.innerHTML = `❌ No backups found for user "${currentUser}"<br><small>Create a backup first</small>`;
            serverStatus.className = 'status error';
            serverStatus.style.display = 'block';
            return;
        }
...
// Version and Admin System (keep existing code)
...
const AdminManager = {
    adminUsers: ['admin', 'default', 'klindakoil'],
    ...
    renderAdminControls(container) {
        ...
        container.innerHTML = `
            ...
            <div class="detail-text" style="font-size: 0.8rem; opacity: 0.8; margin-top: 1rem;">
                ...
                • User management<br>
                • Debug and update controls<br><br>
