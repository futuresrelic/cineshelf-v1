...
error_log("CineShelf API Backup: User '{$user}' -> File '{$filename}'");
error_log("CineShelf API Backup: Input data keys: " . implode(', ', array_keys($input)));
...
if (file_put_contents($filename, json_encode($input, JSON_PRETTY_PRINT))) {
    error_log("CineShelf API Backup: SUCCESS - Wrote to {$filename}");
    echo json_encode([
        'success' => true, 
